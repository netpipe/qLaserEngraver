...
static void process_file(FILE *fin, FILE *fout, const char *infile, const char *outfile) {
  ...
  for (count=0; ; count++) {
    ...
    switch (r) {
    case -1:  /* system error */
   //   fprintf(stderr, ""+ QString(MKBITMAP) +": %s: %s\n", infile, strerror(errno));
      //qDebug() <<  strerror(errno);
      exit(2);
    case -2:  /* corrupt file format */
    //  fprintf(stderr, "" +MKBITMAP +": %s: file format error: %s\n", infile, gm_read_error);
    //  qDebug() << "fileformat error" << QString(gm_read_error);
      exit(2);
    case -3:  /* empty file */
      if (count>0) {  /* end of file */
	return;
      }
   //   fprintf(stderr, ""+ QString(MKBITMAP) +": %s: empty file\n", infile);
   //    qDebug() << "emptyfile";
      exit(2);
    case -4:  /* wrong magic */
      if (count>0) {
//	fprintf(stderr, ": %s: warning: junk at end of file\n", infile);
   //  qDebug() << "warning junk at end of file";
	return;
      }
...
int mainmkbitmap(int ac, char *av[]) {
  FILE *fin, *fout;
  int i;
  char *outfile;
